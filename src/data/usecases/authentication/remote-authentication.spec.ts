...
describe('Remote Authentication', () => {
  ...
  test('Should call HttpPostClient with correct body', async () => {
    ...
    const authencationParams = mockAuthentication()
    await sut.auth(authencationParams)
    expect(httpPostClientSpy.body).toEqual(authencationParams)
  })

  test('Should throw invaliedCredentialsError if HttpPostClient returns 401', async () => {
    const { sut, httpPostClientSpy } = makeSut()
  ...
  test('Should throw invaliedCredentialsError if HttpPostClient return 500', async () => {
    const { sut, httpPostClientSpy } = makeSut()
